...
#
# The seven gated Salesforce tests share one Salesforce org, one user and one connected app. That
# forces them to run sequentially: the Bulk API source reads every Lead in the object, so a
# concurrent test's record gets ingested and copied into the second org, and the PushTopic Source
# and SObject Sink tests create a PushTopic with the same name in that org.
#
# Giving a test its own credentials removes those constraints. Any subset can be provided -
...
#
#   salesforce_use_test_creds KDP_CDC              # primary org only
#   salesforce_use_test_creds KDP_SOBJECT ACCOUNT2 # also the second org
#
# For suffix KDP_CDC this prefers, when set and non-empty:
#   SALESFORCE_USERNAME_KDP_CDC, SALESFORCE_PASSWORD_KDP_CDC, SALESFORCE_SECURITY_TOKEN_KDP_CDC,
#   SALESFORCE_INSTANCE_KDP_CDC, SALESFORCE_CONSUMER_KEY_KDP_CDC, SALESFORCE_CONSUMER_PASSWORD_KDP_CDC,
#   SALESFORCE_CONSUMER_KEY_WITH_JWT_KDP_CDC
# and with ACCOUNT2, the same names suffixed _KDP_CDC_ACCOUNT2 for the second org.
#
# Ported from vdesabou/kafka-docker-playground (commit 63405a1c2). The tests on this branch use
# the username/password OAuth flow, so CONSUMER_KEY and CONSUMER_PASSWORD are overridable here as
# well as the CONSUMER_KEY_WITH_JWT name upstream uses.
#
# Indirect expansion is used deliberately here: this resolves seven names across two orgs, so
# spelling each out would be fourteen near-identical blocks.
function salesforce_use_test_creds() {
  local suffix="$1"
  local also_account2="${2:-}"
  local bases="USERNAME PASSWORD SECURITY_TOKEN INSTANCE CONSUMER_KEY CONSUMER_PASSWORD CONSUMER_KEY_WITH_JWT"
