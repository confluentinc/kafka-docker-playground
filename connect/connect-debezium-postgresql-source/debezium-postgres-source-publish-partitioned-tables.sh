...
log "Don't create a Publication with publish_via_partition_root = true:"
#docker exec -i postgres psql -U myuser -d postgres << EOF
...
log "Create simple OLDCUSTOMERS table:"
docker exec -i postgres psql -U myuser -d postgres << EOF
...
ALTER TABLE OLDCUSTOMERS REPLICA IDENTITY FULL;

INSERT INTO OLDCUSTOMERS (id, first_name, last_name, email, gender, club_status, comments, create_ts) VALUES
(1, 'Alice', 'Smith', 'alice.smith@example.com', 'Female', 'gold', 'First customer', '2022-01-01 10:00:00+00'),
(2, 'Bob', 'Johnson', 'bob.johnson@example.com', 'Male', 'silver', 'Loyal customer', '2022-01-02 11:00:00+00'),
(3, 'Carol', 'Williams', 'carol.williams@example.com', 'Female', 'bronze', 'Frequent buyer', '2022-01-03 12:00:00+00'),
(4, 'David', 'Brown', 'david.brown@example.com', 'Male', 'gold', 'VIP', '2022-01-04 13:00:00+00'),
(5, 'Eve', 'Jones', 'eve.jones@example.com', 'Female', 'silver', 'Newsletter subscriber', '2022-01-05 14:00:00+00'),
(6, 'Frank', 'Garcia', 'frank.garcia@example.com', 'Male', 'bronze', 'Occasional buyer', '2022-01-06 15:00:00+00'),
(7, 'Grace', 'Martinez', 'grace.martinez@example.com', 'Female', 'gold', 'Top spender', '2022-01-07 16:00:00+00'),
(8, 'Henry', 'Rodriguez', 'henry.rodriguez@example.com', 'Male', 'silver', 'Referral', '2022-01-08 17:00:00+00'),
(9, 'Ivy', 'Lee', 'ivy.lee@example.com', 'Female', 'bronze', 'Gift card user', '2022-01-09 18:00:00+00'),
(10, 'Jack', 'Walker', 'jack.walker@example.com', 'Male', 'gold', 'Returning customer', '2022-01-10 19:00:00+00');
EOF

log "Show content of OLDCUSTOMERS table:"
docker exec -i postgres psql -U myuser -d postgres << EOF
...
playground connector create-or-update --connector debezium-postgres-source  << EOF
{
    ...
    "table.include.list": "public.customers,public.oldcustomers",
...
log "Verifying topic asgard.public.oldcustomers-raw"
playground topic consume --topic asgard.public.oldcustomers-raw --min-expected-messages 10 --timeout 60
