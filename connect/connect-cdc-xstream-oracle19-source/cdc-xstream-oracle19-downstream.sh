...
# Pre-built downstream capture Oracle images from ECR.
...
playground start-environment --environment "${PLAYGROUND_ENVIRONMENT}" --docker-compose-override-file "${PWD}/docker-compose.plaintext.downstream.yml"
...
log "Creating Oracle Xstream CDC source connector (downstream capture mode)"
