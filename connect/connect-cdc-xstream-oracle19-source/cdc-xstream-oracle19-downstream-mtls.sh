...
# Parameterized downstream capture mTLS (two-way) test script.
#
# Environment variables:
#   SOURCE_TLS_MODE      - disable | one-way | two-way  (default: two-way)
#   DOWNSTREAM_TLS_MODE  - disable | one-way | two-way  (default: two-way)
#
# Wallets are always separate per database. No shared wallet support.
#
# Test matrix (two-way combinations):
#   #5:  SOURCE_TLS_MODE=two-way   DOWNSTREAM_TLS_MODE=disable   (source wallet only)
#   #6:  SOURCE_TLS_MODE=disable   DOWNSTREAM_TLS_MODE=two-way   (downstream wallet only)
#   #7:  SOURCE_TLS_MODE=two-way   DOWNSTREAM_TLS_MODE=two-way   (separate wallets)
#   #8:  SOURCE_TLS_MODE=one-way   DOWNSTREAM_TLS_MODE=two-way   (mixed, separate)
#   #9:  SOURCE_TLS_MODE=two-way   DOWNSTREAM_TLS_MODE=one-way   (mixed, separate)
#
# Pre-built downstream capture Oracle images with mTLS (two-way) already configured.
...
# AND mTLS wallets/listener pre-configured on port 2484.
...
#
# Two-way TLS image layout:
#   Server wallet (mTLS): /opt/oracle/oradata/dbconfig/{SID}/mwallet (SSL_CLIENT_AUTHENTICATION = TRUE)
#   Two-way client wallet: /opt/oracle/oradata/twoway_client_wallet (client cert + key + trusted CA)
#   TCPS listener on port 2484
#
# One-way TLS image layout (used for mixed tests #10-13):
#   Server wallet: /opt/oracle/oradata/dbconfig/{SID}/wallet (SSL_CLIENT_AUTHENTICATION = FALSE)
#   One-way client wallet: /opt/oracle/oradata/oneway_client_wallet (trusted CA only)
#   TCPS listener on port 2484
#
# Docker network aliases "srccdb" and "capcdb" must match the TNS entries baked into the images.

SOURCE_TLS_MODE=${SOURCE_TLS_MODE:-"two-way"}
DOWNSTREAM_TLS_MODE=${DOWNSTREAM_TLS_MODE:-"two-way"}

# Validate inputs
if [[ "$SOURCE_TLS_MODE" != "disable" && "$SOURCE_TLS_MODE" != "one-way" && "$SOURCE_TLS_MODE" != "two-way" ]]; then
     logerror "Invalid SOURCE_TLS_MODE: $SOURCE_TLS_MODE (must be disable, one-way, or two-way)"
     exit 1
fi
if [[ "$DOWNSTREAM_TLS_MODE" != "disable" && "$DOWNSTREAM_TLS_MODE" != "one-way" && "$DOWNSTREAM_TLS_MODE" != "two-way" ]]; then
     logerror "Invalid DOWNSTREAM_TLS_MODE: $DOWNSTREAM_TLS_MODE (must be disable, one-way, or two-way)"
     exit 1
fi

log "mTLS test configuration: SOURCE_TLS_MODE=$SOURCE_TLS_MODE, DOWNSTREAM_TLS_MODE=$DOWNSTREAM_TLS_MODE"
...
# two-way -> -source-real-tls-twoway / -capture-real-tls-twoway
if [[ "$SOURCE_TLS_MODE" == "two-way" ]]; then
     export ORACLE_IMAGE_SRC="${ORACLE_ECR_REPO}:19.29.0-ee-source-real-tls-twoway"
elif [[ "$SOURCE_TLS_MODE" == "one-way" ]]; then
...
if [[ "$DOWNSTREAM_TLS_MODE" == "two-way" ]]; then
     export ORACLE_IMAGE_CAP="${ORACLE_ECR_REPO}:19.29.0-ee-capture-real-tls-twoway"
elif [[ "$DOWNSTREAM_TLS_MODE" == "one-way" ]]; then
...
if [[ "$SOURCE_TLS_MODE" != "disable" ]]; then
     SOURCE_PORT="2484"
else
     SOURCE_PORT="1521"
fi
if [[ "$DOWNSTREAM_TLS_MODE" != "disable" ]]; then
...
# Helper: determine client wallet path inside a container based on TLS mode
# two-way images: /opt/oracle/oradata/twoway_client_wallet/cwallet.sso
# one-way images: /opt/oracle/oradata/oneway_client_wallet/cwallet.sso
get_client_wallet_path() {
     local tls_mode=$1
     if [[ "$tls_mode" == "two-way" ]]; then
          echo "/opt/oracle/oradata/twoway_client_wallet/cwallet.sso"
     else
          echo "/opt/oracle/oradata/oneway_client_wallet/cwallet.sso"
     fi
}
...
if [[ "$SOURCE_TLS_MODE" == "disable" && "$DOWNSTREAM_TLS_MODE" == "disable" ]]; then
     ...
     if [[ "$SOURCE_TLS_MODE" != "disable" ]]; then
          SRC_WALLET_CONTAINER_PATH=$(get_client_wallet_path "$SOURCE_TLS_MODE")
          log "🔏 copy ${SOURCE_TLS_MODE} client cwallet.sso from sourcedb to connect container (source wallet)"
          playground container exec --root --command "mkdir -p /tmp/source_wallet"
          docker cp sourcedb:${SRC_WALLET_CONTAINER_PATH} /tmp/cwallet_source.sso
     ...
     if [[ "$DOWNSTREAM_TLS_MODE" != "disable" ]]; then
          DS_WALLET_CONTAINER_PATH=$(get_client_wallet_path "$DOWNSTREAM_TLS_MODE")
          log "🔏 copy ${DOWNSTREAM_TLS_MODE} client cwallet.sso from downstreamdb to connect container (downstream wallet)"
          playground container exec --root --command "mkdir -p /tmp/downstream_wallet"
          docker cp downstreamdb:${DS_WALLET_CONTAINER_PATH} /tmp/cwallet_downstream.sso
...
SOURCE_TLS_CONFIG=""
if [[ "$SOURCE_TLS_MODE" != "disable" ]]; then
     SOURCE_TLS_CONFIG="\"database.tls.mode\": \"${SOURCE_TLS_MODE}\",
	\"database.wallet.location\": \"${SOURCE_WALLET_PATH}\","
fi

DOWNSTREAM_TLS_CONFIG=""
if [[ "$DOWNSTREAM_TLS_MODE" != "disable" ]]; then
     DOWNSTREAM_TLS_CONFIG="\"downstream.database.tls.mode\": \"${DOWNSTREAM_TLS_MODE}\",
